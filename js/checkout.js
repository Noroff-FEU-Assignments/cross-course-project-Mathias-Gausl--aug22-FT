...
next_btn.addEventListener("click", (click) => {
  ...
  if (
    inputLength(fullName, 8) === true &&
    inputLength(number, 8) === true &&
    emailVal() === true
  ) {
...
submit.addEventListener("click", (click) => {
  click.preventDefault();
  if (
    inputLengthStatic(card, 16) === true &&
    inputLengthStatic(exp, 4) === true &&
    inputLengthStatic(cvc, 3) === true
  ) {
...
next_btn.addEventListener("click", () => {
  invalidInput(
    number,
    numberLabel,
    "Phone number needs to be a minimum of 8 digits",
    "Phone number",
    8
  );
});
...
next_btn2.addEventListener("click", () => {
  invalidInput(
    adress,
    adressLabel,
    "Adress needs to contain 5 or more characters ",
    "Adress",
    5
  );
});
next_btn2.addEventListener("click", () => {
  invalidInput(
    zip,
    zipLabel,
    "Zip-code should consist of 4 digits",
    "Zip-code",
    4
  );
});
...
submit.addEventListener("click", () => {
  invalidInputStatic(
    exp,
    expLabel,
    "Expiration date should consist of 4 digits",
    "Expiration date",
    4
  );
});
submit.addEventListener("click", () => {
  invalidInputStatic(
    cvc,
    cvcLabel,
    "CVC should consist of 3 digits",
    "cvc/security code",
    3
  );
});
